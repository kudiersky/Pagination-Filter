...
//Real time validation on line 155
//form validation from line 168
//payment details div line 127
//conflicting activities line 89 & 111
//Total cost line 56
//design and colors line 35
//other info line 15
...
var cvvErrorMessage = 'Please enter your CVV';
 ...
 if($("#name").val() === "" && $('#nameErrorMessage').length < 1){                                            //if the name is blank as there are no error messages for this
  ...
  $('#name').keyup(function() {
    $('#nameErrorMessage').remove()                                                                           //Real time Error remove on key-up
  })
 ...
 if($("#mail").val() === "" && $('#mailErrorMessage').length < 1 ){                                               //if the name is blank as there are no error messages for this
    ...
    $('#mail').keyup(function() {
      $('#mailErrorMessage').remove()
                                                                                                      //Real time Error remove on key

    })


                                                                                                        //for selecting Role
 if($("title").val() === ""&& $('#selectErrorMessage').length < 1 ){                                    //if the name is blank as there are no error messages for this
$("#title").before( "<p id='selectErrorMessage'class='error'> select an option </p>");
errors = true;                                                                                         //log as error
}
 ...
 if (($('#other-title').val() === otherInfo || $('#other-title').val() === '') && $('#emptyOptionErrorMessage').length < 1 ) {    //this ensure the error message only shows when credit card is selected
 $("#selectErrorMessage").replaceWith( "<p id='emptyOptionErrorMessage'><span class='error'> " + emptyOptionErrorMessage + " </span></p>");  //dynamically create error
 errors = true;
 }

    else{
    $('#emptyOptionErrorMessage').remove()                                                                              //remove error
    }


 if ($('input:checked').length < 1 && $('#activitiesErrorMessage').length < 1 ){                                        //if no input has been selected error & no other instance of error
 $(".activities legend").before( "<p id='activitiesErrorMessage' class='error'> please select an activity</p>");       //dynamically create error
 errors = true;                                                                                                        //log as error
 }
